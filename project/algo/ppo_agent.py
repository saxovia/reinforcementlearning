...
class PPOAgent(BaseAgent):
    def __init__(self, config=None):
        ...
        self.policy= Policy(self.env.observation_space.shape[0], self.env.action_space.shape[0], self.cfg)
    ...
    def get_action(self, observation, evaluation=False):  # Updated
        ...
        if action.numel() == 1:
            action = action.item() # one action
        else:
            action = action.detach().cpu().numpy()  # detach + convert to numpy
    ...
    def train_iteration(self,ratio_of_episodes):
        ...
        while not done and episode_length < self.cfg.max_episode_steps:
            ...
            if len(self.states) > self.cfg.min_update_samples:
                ...
                #self.policy.actor_logstd.data = torch.clamp(self.policy.actor_logstd * ratio_of_episodes, min=-2.0, max=0.5)
